...
import useSortedData from "../path-to-your-hook";
...
describe("useSortedData", () => {
	...
	test("sorts data by name in ascending order", () => {
		const sortConfig = { key: "firstName", direction: "ascending" };
		const { result } = renderHook(() => useSortedData(jsonData, sortConfig));
		act(() => {
	...
	test("sorts data by name in descending order", () => {
		const sortConfig = { key: "firstName", direction: "descending" };
		const { result } = renderHook(() => useSortedData(jsonData, sortConfig));
		act(() => {
			expect(result.current).toEqual([
				{
	...
	test("sorts data by date in ascending order", () => {
		const sortConfig = { key: "startDate", direction: "ascending" };
		const { result } = renderHook(() => useSortedData(jsonData, sortConfig));
		act(() => {
			expect(result.current).toEqual([
				{
	...
	test("sorts data by date in descending order", () => {
		const sortConfig = { key: "startDate", direction: "descending" };
		const { result } = renderHook(() => useSortedData(jsonData, sortConfig));
	...
	test("returns data unchanged if sort key is defaultKey", () => {
		const sortConfig = { key: "defaultKey", direction: "ascending" };
		const { result } = renderHook(() => useSortedData(jsonData, sortConfig));

		act(() => {
			expect(result.current).toEqual(jsonData);
		});
	});
	...
	test("handles null values correctly", () => {
		...
		const sortConfig = { key: "firstName", direction: "ascending" };
		const { result } = renderHook(() => useSortedData(jsonDataWithNulls, sortConfig));
		act(() => {
			expect(result.current).toEqual([
				{
					id: "168hYxdqWAyB4VMEdb9df1KPyRjTKTC39t",
...
import DataTable from "../lib/dataTable/DataTable"; // Assurez-vous que le chemin est correct
